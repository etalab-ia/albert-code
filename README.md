...
| Variable | Défaut | Rôle |
|---|---|---|
| `AC_ALBERT_BASE_URL` | `https://albert.api.etalab.gouv.fr/v1` | Racine OpenAI-compatible, écrite comme `baseURL` du provider dans `opencode.json`. Elle sert aussi de cible à l'appel catalogue `GET <base>/models` tant que le projet n'est pas configuré. Elle doit commencer par `http://` ou `https://`, sans guillemet, antislash ni espace, sinon la commande s'arrête. En `http://`, ta clé circule en clair, ce qu'un avertissement rappelle à chaque commande. |

La valeur est résolue au `setup` et écrite en clair dans l'`opencode.json` généré, jamais en `{env:...}`, car OpenCode tourne dans la VM et n'y reçoit pas cette variable. Tu ne la positionnes donc qu'une fois : `albert-code run` utilise le `baseURL` écrit dans le projet et `albert-code update` le relit pour vérifier le catalogue de modèles, sans le remettre au défaut. Pour revenir à Albert, modifie l'`opencode.json` ; relancer un `setup` sans la variable ne suffit pas, car un projet qui déclare déjà le provider Albert passe en simple vérification et garde son `baseURL` (un avertissement le signale si la variable est positionnée et diffère).

> ⚠️ **Confirmation avant d'envoyer ta clé** : la vérification du catalogue envoie ta clé `ALBERT_API_KEY` au `baseURL` du projet. Comme l'`opencode.json` est versionné, un dépôt cloné pourrait déclarer n'importe quel endpoint. Quand ce `baseURL` diffère de `AC_ALBERT_BASE_URL` (ou du défaut si la variable est absente), `setup` et `update` affichent l'URL et demandent confirmation avant l'appel ; si tu refuses, la vérification est ignorée et la configuration conservée. Sur un projet à `baseURL` personnalisé, la question revient donc à chaque `update` lancé sans la variable : lance-le avec `AC_ALBERT_BASE_URL` égale au `baseURL` du projet pour ne pas être interrogé.
