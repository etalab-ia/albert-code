...
- **Provider Albert** : `provider.albert.npm = "@ai-sdk/openai-compatible"`, `options.baseURL = "https://albert.api.etalab.gouv.fr/v1"` (défaut, surchargeable par `AC_ALBERT_BASE_URL`, cf. T1.9), `options.apiKey = "{env:ALBERT_API_KEY}"`, `models` listant uniquement `deepseek-v4-flash`. Puis `model = "albert/deepseek-v4-flash"`, `small_model = "albert/deepseek-v4-flash"`.
