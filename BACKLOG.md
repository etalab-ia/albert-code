...
**But :** remplacer partout les identifiants morts par les id canoniques en minuscules ; DeepSeek par défaut ; retirer `Qwen/Qwen3.6-27B` (le bundle ne livrant que `deepseek-v4-flash`) et toute mention de multimodal/vision (le bundle n'a plus de modèle de vision).
...
**✅ Implémenté** — consigne ajoutée dans la section « Git & commits » (`templates/AGENTS.default.md:57`, trailer `Co-Authored-By: Albert Code (<id-modèle>) <noreply@numerique.gouv.fr>`, précédé d'une ligne vide). **Source déclarative, non vérifiable** : la version d'OpenCode embarquée n'expose aucun identifiant du modèle courant à l'agent (aucune variable d'env type `OPENCODE_MODEL`, aucune substitution dans le contenu d'`AGENTS.md` — seules `{env:}`/`{file:}` existent et uniquement dans `opencode.json` ; le contexte système injecte seulement cwd/root/git/plateforme/date). La consigne demande donc à l'agent de noter l'**id canonique Albert** du modèle tel qu'il le connaît ; l'EPIC 11 multipliera les rôles de modèles, mais ce garde-fou reste faible (à compléter par un hook de pré-commit en T10.5). → `TESTS.md` S53.
...
- **C. Bloquer ou avertir → bloquer sur le nom exact (faible bruit), avertir sur les patterns flous.** Le principe du ticket est respecté : un hook qui refuse à tort apprend à passer `--no-verify` par réflexe et désarme le garde-fou. Or la détection par nom de variable est à très faible taux de faux positifs (un `ALBERT_API_KEY=` est sans ambiguïté) : sur elle, **refus**. Sur les motifs de formats ambigus (la regex de conformity-check), on se contente d'un **avertissement** pour ne jamais bloquer un commit légitime.
...
**But :** donner corps à la décision T10.5 : poser le garde-fou de détection de secrets déclaré par `templates/AGENTS.default.md:8` sur les projets scaffoldés des utilisateurs, par détection de secret **au nom de la variable en contexte** (pas au format).
**Tâches :**
- Écrire `.githooks/pre-commit` (script bash, `set -euo pipefail`, compatible 3.2, idempotent) qui scrute le **diff stagé** (`git diff --cached`) et refuse si une ligne d'assignation d'un secret connu apparaît : `ALBERT_API_KEY=`, `GH_TOKEN=`, `CONTEXT7_API_KEY=` (et variables d'identité). Refus (exit ≠ 0) sur nom exact ; simple avertissement sur les motifs de formats ambigus (regex de `conformity-check.sh`). Ne jamais bloquer sur un motif flou.
...
- Ajouter au script du bundle la récupération d'`ALBERT_API_KEY` au champ `id` canonique via `GET /v1/models` **uniquement** (jamais un alias), conformément à la règle §10.
- Tests dans `TESTS.md` : un commit contenant `ALBERT_API_KEY=…` est refusé (exit ≠ 0) ; `--no-verify` le laisse passer ; un commit sans secret de nom connu passe ; le motif flou ne bloque pas.
**DoD :** sur un projet scaffoldé, committer un fichier contenant `ALBERT_API_KEY=sk-…` est **refusé** par le hook (hors `--no-verify`) ; un commit sain passe ; la consigne `AGENTS.default.md` est vraie (le garde-fou décrit est réellement installé au setup). → scénario `TESTS.md` à créer (S61).
**← décision T10.5 (2026-08-27).**

### T10.7 🟠 Rétention et purge de la base de session OpenCode dans la VM
**Problème :** la base de session d'OpenCode dans la VM (`~/.local/share/opencode/opencode.db`) accumule les sorties d'outils. Tout secret qui a transité une seule fois dans le contexte de la session y **persiste au repos** et peut être re-résumé vers le modèle aux tours suivants. C'est un risque distinct de T10.5, observable lors de l'incident du 26/08 : le secret avait transité par la sortie d'un outil vers le contexte de l'agent, or un hook de pré-commit n'intercepte que ce qu'on écrit dans un fichier — la persistance en base de session, elle, n'est couverte par aucun garde-fou existant ni pas la push protection forge (le secret ne passe jamais par git).
**But :** instruire la **rétention** (durée de vie de la base, ce qui y est stocké : historiques de conversation, sorties d'outils, environnement) et la **purge** (rotation / nettoyage de la base en fin de session ou de VM) de cette base, et trancher la bonne politique pour le bundle.
**Tâches :** cartographier la base (`sqlite`, tables, ce qui y persiste) ; décider la politique de rétention défendable pour un outil manipulé par des agents publics (durée de vie, purge au boot ou au reset de VM) ; documenter et implémenter la purge (script de nettoyage idempotent, exécuté au setup/run ou au démarrage de la VM).
