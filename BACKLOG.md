...
**But :** donner corps à la décision T10.5 : poser le garde-fou de détection de secrets déclaré par `templates/AGENTS.default.md:8` sur les projets scaffoldés des utilisateurs, par détection d'un secret **au nom de la variable plus une valeur plausible** en contexte (pas au format seul).
**Tâches :**
...
**Tâches :** traiter les trois facettes ; ajouter des scénarios `TESTS.md` (S64 et suivants, en coordination avec T8.5 qui réserve déjà cette plage) couvrant explicitement le remplacement d'un secret valide par un autre secret valide, cas absent de la suite actuelle.

**DoD :** remplacer un jeton valide par un autre se fait par le wizard, de bout en bout jusqu'à la VM ; toute divergence entre environnement et fichier est signalée ; la CLI `gh` de l'utilisateur n'est plus contrainte par le jeton de l'agent.
